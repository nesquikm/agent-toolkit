...
`claude --session-id <uuid>`. Nobody else can produce the value you generated, so
the id cannot select a session you did not start. The name rides along for the
humans; the uuid is what every lookup actually resolves through.

**It is the *value* that is yours, not the flag** — and the difference is not
pedantry, it is the whole join. This file said for eight releases that a human
never passes `--session-id`. Measured 2026-08-17 on this machine: of 9 live
sessions started by hand (no `-n`, so auto-named), **7 carried a `--session-id`
nobody typed**, injected by a launcher wrapping the CLI. The session reading this
was one of them. The guarantee is intact, because an injected id is freshly random
and yours is generated by you — but it rests on unguessability, never on absence.
So never reduce an ownership test to *does this session have a session id*, or
*does its command line mention the flag*: on this machine that check passes
everything, including the user's own terminals.
...
A uuid you generated cannot be produced by anyone else, so it can never select a
hand-started session. That is the whole guarantee, and it is why the mint is not
optional. **The guarantee is unguessability, not the absence of the flag** — see
the top of this file for the measurement, where 7 of 9 hand-started sessions on
this machine carried an injected `--session-id`. A test that asks whether a
session has one at all is not an ownership test.
