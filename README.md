...
- **Push-based waiting.** A polling loop only runs while you are inside a turn, so any stage that outlives the turn is a stage nobody is watching. The bundled `watch-workers.py` polls Claude Code's peer registry, filtered to this run's workers, and emits one line per state change (`DONE` / `GATE` / `ASK` / `ATTN` / `CLEAR` / `GONE`), each of which re-invokes the orchestrator. `GATE` is the one that is not read off the registry: a worker that asks its question *in prose* and ends its turn registers `idle`, byte-identical to one that finished, so the watcher reads the tail of its transcript and quotes the closing words back to you — the difference between a supervisor that can route an approval gate and a human who has to notice the stall. A seventh line, `WARN`, is not a worker signal at all — it names no worker, and reports on the watcher itself: that it matches no live session and is therefore watching nothing (once, about 30 s in), or that its gate predicate raised and every `GATE` is degrading to a bare `DONE` until it is fixed.
